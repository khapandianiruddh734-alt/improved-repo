...
/**
 * Generic Sheet Exporter with optional highlight support.
 * Pass a Set of "row-col" keys (0-based) to tint edited cells.
 */
export function exportToExcel(data: any[][], filename: string, highlightCells?: Set<string>) {
  const withStyles = Array.isArray(data)
    ? data.map((row, r) =>
        Array.isArray(row)
          ? row.map((cell, c) => {
              const styled: any = { v: cell, t: typeof cell === 'number' ? 'n' : 's' };
              if (highlightCells && highlightCells.has(`${r}-${c}`)) {
                styled.s = {
  ...
  const ws = XLSX.utils.aoa_to_sheet(withStyles);
