...
export const Workspace: React.FC<WorkspaceProps> = ({ tool }) => {
  ...
  const [originalGrid, setOriginalGrid] = useState<any[][] | null>(null);
  ...
  // Build a quick lookup of which cells were altered so we can highlight them in the UI.
  const computeDiffSet = (original: any[][], updated: any[][]): Set<string> => {
    const diff = new Set<string>();
    const rowCount = Math.max(original.length, updated.length);
    for (let r = 0; r < rowCount; r++) {
      const origRow = original[r] || [];
      const newRow = updated[r] || [];
      const colCount = Math.max(origRow.length, newRow.length);
      for (let c = 0; c < colCount; c++) {
        const a = String(origRow[c] ?? "").trim();
        const b = String(newRow[c] ?? "").trim();
        if (a !== b) diff.add(`${r}-${c}`);
      }
    }
    return diff;
    ...
    try {
      ...
      switch (tool.id) {
        ...
        case 'ai-menu-fixer': {
          ...
          setChangedCells(computeDiffSet(grid, restructuredData));
  ...
  const handleDownload = () => {
    if (previewData) {
      const highlights = tool.id === 'ai-menu-fixer' && changedCells.size > 0 ? changedCells : undefined;
      dataService.exportToExcel(previewData, `achievers_${tool.id}_export.xlsx`, highlights);
      return;
    }
                ...
                <div className="w-full bg-slate-50 p-8 rounded-[3rem] border border-slate-200 mb-6 max-h-[600px] overflow-y-auto">
                 ...
                 <div className="w-full">
                    <div className="flex justify-between items-center mb-2">
                      <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest">Operation Results</h4>
                      {tool.id === 'ai-menu-fixer' && changedCells.size > 0 && (
                       ...
                       <div className="overflow-x-auto">
                          <table className="w-full text-left text-[10px] font-mono border-collapse">
                             <thead className="bg-slate-200/50">
                                <tr>
                                  {previewData[0]?.slice(0, 8).map((h, i) => {
                                    const isChanged = tool.id === 'ai-menu-fixer' && changedCells.has(`0-${i}`);
                                    return (
                                      <th 
                                        key={i} 
                                        className={`px-4 py-3 border-b border-slate-200 font-black uppercase ${isChanged ? 'bg-amber-100 text-amber-900' : 'text-slate-500'}`}
                                      >
                                        {h}
                                      </th>
                                    );
                                ...
                                {previewData.slice(1, 11).map((row, ri) => (
                                   <tr key={ri} className="bg-white border-b border-slate-100 last:border-0 hover:bg-indigo-50/50">
                                      {row.slice(0, 8).map((cell, ci) => {
                                        const absoluteRow = ri + 1; // +1 to account for header row at index 0
                                        const isChanged = tool.id === 'ai-menu-fixer' && changedCells.has(`${absoluteRow}-${ci}`);
                                        return (
                                          <td 
                                            key={ci} 
                                            className={`px-4 py-3 truncate max-w-[200px] ${isChanged ? 'bg-amber-50 text-amber-900 font-semibold ring-1 ring-amber-200' : 'text-slate-600'}`}
                                          >
                                            {cell}
                                          </td>
                                        );
